...
function M.count(state, on_error, on_count)
  M.run(pipeline.count_command(state), on_error, function(stdout)
...
function M.frequency(state, column, on_error, on_values)
  M.run_json_lines(pipeline.frequency_command(state, column), on_error, function(rows)
    local values = {}
...
function M.stats(state, column, on_error, on_stats)
  M.run_json_lines(pipeline.stats_command(state, column), on_error, function(rows)
    if #rows == 0 then
